...
#
#
# def preprocess_supervised_dataset_sup(
#     examples: Dict[str, List[Any]],
#     tokenizer: "PreTrainedTokenizer",
#     template: "Template",
#     data_args: "DataArguments",
...
#     # for multiturn examples, we only mask the prompt part in each prompt-response pair.
#     model_inputs = {"input_ids": [], "attention_mask": [], "labels": [],
#                     "input_ids_wo_labels": [], "attention_mask_wo_labels": []}
#
#     for i in range(len(examples["prompt"])):
#         if len(examples["prompt"][i]) % 2 != 1 or len(examples["response"][i]) != 1:
#             continue
...
#             input_ids += source_ids + target_ids
#             labels += source_mask + target_ids
#             input_ids_wo_labels += source_ids
#
#         if template.efficient_eos:
#             input_ids += [tokenizer.eos_token_id]
#             labels += [tokenizer.eos_token_id]
#             input_ids_wo_labels += [tokenizer.eos_token_id]
#
#         model_inputs["input_ids"].append(input_ids)
#         model_inputs["attention_mask"].append([1] * len(input_ids))
#         model_inputs["labels"].append(labels)
#         model_inputs["input_ids_wo_labels"].append(input_ids_wo_labels)
#         model_inputs["attention_mask_wo_labels"].append([1] * len(input_ids_wo_labels))
#
#     return model_inputs
...
def get_preprocess_and_print_func(
    ...
    stage: Literal["pt", "sft", "rm", "ppo", "align", "sup"],
    ...
    elif stage == "sup" and not training_args.predict_with_generate:
        if data_args.sft_packing:
            preprocess_func = partial(
                preprocess_packed_supervised_dataset, tokenizer=tokenizer, template=template, data_args=data_args
            )
