...
def evaluate(gold, predict, db_dir, etype, kmaps, plug_value, keep_distinct, progress_bar_for_each_datapoint, diff_json_path):
    ...
    diff_json = json.load(open(diff_json_path))
    new_diff_json = []
    new_diff_json_path = "./data/sft_data_collections/spider/dev.json"
    ...
    for i, (p, g) in enumerate(zip(plist, glist)):
        ...
        for idx, pg in enumerate(zip(p, g)):
            this_instance = diff_json[idx]
            # print(idx)
            # print(this_instance["db_id"])
            # print(this_instance["query"])
            # print(g)
            p, g = pg
            p_str = p[0]
            ...
            # print(hardness)
            # print("\n")
            this_instance["difficulty"] = hardness
            new_diff_json.append(this_instance)
            if idx > 3:
                idx = "> 4"
            else:
                idx += 1
    ...
    json.dump(new_diff_json, open(new_diff_json_path, 'w'), indent=4)
...
if __name__ == "__main__":
    ...
    evaluate(args.gold, args.pred, args.db, args.etype, kmaps, args.plug_value, args.keep_distinct,
             args.progress_bar_for_each_datapoint, args.diff_json_path)
