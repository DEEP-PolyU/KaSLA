...
from transformers import AutoConfig, RobertaModel, AutoModel, GPTNeoForCausalLM
...
class SchemaItemClassifier(nn.Module):
    def __init__(self, model_name_or_path, mode):
        ...
        if mode in ["eval", "test"]:
            ...
            if "SGPT" in model_name_or_path:
                self.plm_encoder = AutoModel.from_pretrained(model_name_or_path)
            else:
    ...
    def table_column_cls(
        ...
        for batch_id in range(batch_size):
            ...
            # if "SGPT" in self.model_name_or_path:
